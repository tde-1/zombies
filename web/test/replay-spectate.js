...
async function main() {
  ...
  // The transition checks below were written from a third-person start; they test the moves,
  // not the default, so they keep that start explicitly.
  const init3p = (t) => ({ ...S.initSpectate(t), mode: 'follow', view: 'follow' })

  await check('init: the first player, first person (B 2026-09-24)', () => {
    same(S.initSpectate({ players: [P(2), P(0)] }), { focus: 2, mode: 'eyes', view: 'eyes' })
    same(S.initSpectate(null), { focus: 0, mode: 'eyes', view: 'eyes' })
  })

  await check('select: clicking a row changes who; from free cam it starts following the last view', () => {
    const s0 = init3p({ players: FOUR })
  ...
  await check('number keys 1-4 pick panel rows in co-op; 1/2/3 stay camera modes in solo', () => {
    ...
    const s = run(init3p({ players: FOUR }), { type: 'index', n: 2, players: FOUR })
    eq(s.focus, 2)
  ...
  await check('cycle skips downed players while anybody else is up, and falls back when nobody is', () => {
    ...
    s = S.spectate(init3p({ players: allDown }), { type: 'cycle', dir: 1, players: allDown.map((p) => ({ ...p, alive: false })) })
    eq(s.focus, 1, 'everybody down: the next one anyway')
    // a number key still reaches a downed player on purpose
    eq(run(init3p({ players: ps }), { type: 'index', n: 1, players: ps }).focus, 1)
  })
  ...
  await check('Esc lets go to free cam; in free cam Q/E are the fly keys, not a cycle', () => {
    ...
    const s = run(init3p({ players: FOUR }), { type: 'select', slot: 3 }, { type: 'free' })
  ...
  await check('downed: the camera stays on him, the prompt offers the next player who is up', () => {
    ...
    const s = run(init3p({ players: ps }), { type: 'select', slot: 1 })
    eq(s.focus, 1)
  ...
  await check('the follow target persists across a seek or a pause (nothing time-shaped in the state)', () => {
    const s = run(init3p({ players: FOUR }), { type: 'select', slot: 2 }, { type: 'toggle-view' })
