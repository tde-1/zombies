...
def bucket_check(bsps):
    """{bsp: {"in_bucket": bool, "files": n, "bytes": b, "missing": [...]}} -- an anonymous HEAD
    of every served file's public bucket URL, size compared. lib/maps.js offers a box-proven map
    only when this is true: the box pulls maps from the bucket on demand (host.md)."""
    import concurrent.futures
    import urllib.parse
    import urllib.request
    ext = load("extract.json", [])
    want = {}
    for e in ext:
        for m in e.get("mods") or []:
            if m.get("map") in bsps:
                fs = []
                for f in m.get("files") or []:
                    rel = f["path"].split("/", 2)[2]
                    if os.path.splitext(rel)[1].lower() in SERVED_EXT:
                        fs.append((rel, f["size"]))
                want[m["map"]] = (m.get("bsp") or m["map"], fs)

    def head(job):
        key_bsp, rel, size = job
        url = BUCKET_BASE + "/" + urllib.parse.quote(key_bsp + "/" + rel)
        try:
            r = urllib.request.urlopen(urllib.request.Request(url, method="HEAD"), timeout=20)
            return int(r.headers.get("Content-Length") or -1) == size
        except Exception:
            return False
    out = {}
    with concurrent.futures.ThreadPoolExecutor(8) as ex:
        for bsp in bsps:
            if bsp not in want:
                out[bsp] = {"in_bucket": False, "files": 0, "bytes": 0, "missing": ["(no extract.json entry)"]}
                continue
            key_bsp, fs = want[bsp]
            oks = list(ex.map(head, [(key_bsp, rel, sz) for rel, sz in fs]))
            miss = [rel for (rel, _), ok in zip(fs, oks) if not ok]
            out[bsp] = {"in_bucket": bool(fs) and not miss, "files": len(fs),
                        "bytes": sum(sz for _, sz in fs), "missing": miss[:5]}
    return out
    ...
    for bsp, r in sorted(proof.items()):
        if r.get("result") == "pass":
            ...
            if not inb[bsp]["in_bucket"]:
                ...
                print("  NOT offered, files missing from the bucket: %s %s" % (bsp, inb[bsp]["missing"]))
...
def apply():
    ...
    for bsp, r in proof.items():
        ...
        m["box_proof"] = box
        # Phase 2 of a tranche (archive.md s12): the result is in, so the row the phase-1 import
