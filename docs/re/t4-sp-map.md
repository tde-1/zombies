...
| 4 | the co-op join gate | `cmp byte [eax+0x10],0 / je` at **0x62EBC9**, dvar ptr at **0x339A774** | `SV_DirectConnect` refuses with `EXE_ERR_CANNOTJOININPROGRESS` when the dvar is 0. Read twice (0x62E9BB, 0x62EBC4) | set the dvar. **Its name is `party_joinInProgressAllowed`**. **CORRECTED 2026-09-23 (dedi.md §21):** it IS written, at registration: `xor al,al; mov edi,"party_joinInProgressAllowed"; call Dvar_RegisterBool 0x5EEE20; mov [0x339A774],eax` at **0x654D3C..0x654D48** in the party dvar block 0x654530 (Com_Init via 0x5FB560). On today's component order it is already registered at `post_init`. Since 81086d4: default patched to 1 (`B0 01` at 0x654D3C), value set at post_init, and both reads made unconditional (`EB 0A` at 0x62E9BB, `EB 0D` at 0x62EBC4) |
...
| `clc.connectTime` | 0x3010010 | [V] | CL_CheckForResend 0x642CC9: resends in state 4/5 when `cls.realtime - connectTime >= 3000` (0xBB8), then stores realtime there (0x642CE2) |
| `cls.realtime` | 0x48AE4E8 | [V] | read at 0x642CB7/0x642CC9 against connectTime |
| `clc.connectPacketCount` | 0x3010014 | [V] | `add [0x3010014],1` at 0x642CF9 |
| OOB `error`/`lobbyerror` → Com_Error | call at **0x643D50** | [V] | both replies end at 0x643D48 `push esi(msg); push "%s"; push 1; call Com_Error; add esp,0xC; mov al,1` (the `error` branch jumps back at 0x643DF6). `join_retry.cpp` retargets this one call |
| `SCR_DrawScreenField` | 0x478DC0 | [V] | cdecl(int stereo), one caller `push esi; call; add esp,4` at **0x479271** (0x479200). Jump table on clc.state; state 4/5 draws the connect screen 0x5D7D40. Draws nothing while `[0x48AE4DC]` is 0 |
| `UI_DrawConnectScreen` | 0x5D7D40 | [V] | `EXE_CONNECTINGTO` (0x87B95C) / `EXE_DOWNLOADINGUPDATE`; eax = localClientNum; text through 0x5D7580 on **scrPlaceFull 0x957360** (= scrPlaceView 0x957318 + 0x48) |
| `SV_DirectConnect` refusal strings | 0x886D04.. | [V] | `error
EXE_SERVER_IS_DIFFERENT_VER`, `EXE_BAD_CHALLENGE` (0x886DA4), `EXE_ERR_HIGH_PING_ONLY`/`LOW_PING_ONLY`, `EXE_ERR_QPORT`, `EXE_ERR_HOSTALREADYCONNECTED` (0x886EB0), `EXE_SERVERISFULL` (0x886ED4), `EXE_ERR_CANNOTJOININPROGRESS` (0x886F50) |
