...
| *(map_loaded, 2026-09-24)* | `reconnect: 1` | this DLL sends `player_lost`/`player_back`/`player_ready`/`restored`, carries `state` on `player_disconnect`, lets the same account back in past its own lost slot (the ghost is kicked instead of the new connection refused), and queues `restore`. Absent = an older DLL: a host counts a returning player down at `player_connect`. |
...
| `game_over` | `round, reason, duration_ms, points_total, downs_total, players_alive, players:[{slot,name,connected,identity,steamid?,xuid?,party_slot?,identity_reason?,score,score_total,downs,revives,kills?,headshots?,assists?,alive}]`, `kills_total?` | game over — **the final result**, in one message, so a host that loses the link a second later still has the whole answer without re-folding the stream. It is the last event of the match and the replay sampler stops on it. **A `steamid` appears on a player row here only when `identity` is `verified`** — the one message a host may post a result from must not carry an account nobody checked. *(fields added 2026-09-22, referee.md §10.2; identity 2026-09-22, §13; `kills`/`headshots`/`assists`/`kills_total` 2026-09-23, §16 — present only when the DLL's native scoreboard fields verified, absent (not 0) otherwise; `score`/`downs` are now real on a real game; `flags?:[string]` 2026-09-23, dedi.md §23 — flags the game raises about itself. Only `server_freeze` is defined: the DLL's freeze watchdog ended the match because `com_frameTime` stood still for 5 s while frames were entered; `reason` is then `server_freeze` and the result is the state at the freeze. The host copies known flags onto the record (`GAME_FLAGS` in `lib/referee.js`) and ignores the rest)* |
| `match_end` | `round, reason, duration_ms, replay_closed, server_alive, awaiting` | sent immediately **after** `game_over`. Means one thing: **this game process is idle and the instance can be reclaimed.** The server is still alive (`no_save_reload.cpp`) and will sit there for ever unless the host acts. The host must close and sign the replay, post the result, and then either send `end` (reuse: the referee `map_restart`s, resets and re-announces `map_loaded`) or terminate the process. It must do one of the two. **`server_alive:false`** (with `awaiting:"teardown"`) means the process cannot run another match — sent by the freeze watchdog — and the host must terminate it, never `end` it. *(added 2026-09-22, referee.md §10.3; `server_alive:false` 2026-09-23, dedi.md §23)* |
...
| `player_spawn` / `player_disconnect` | `slot` (+`reason`) | `player_disconnect` may also carry `lost:true` (the slot had been called `player_lost` first) and `state` (the player's state as of their last input, the same object as `player_lost.state`), when the DLL has reconnect support (`map_loaded.reconnect`). `reason` `re-seated with a new token`: the engine reused the slot for a new connection without it ever reading inactive; a `player_connect` for the same slot follows. *(state/lost added 2026-09-24, reconnect)* |
| `player_lost` | `slot, silent_ms, state?` | **no input from this slot for `silent_ms` (default 5000, `ENW_RECONNECT_LOST_MS`) while the world ran**: the client crashed, froze or lost its network. The body is still seated -- the engine drops it only after its own ~40 s timeout, and not at all while the world is frozen. `state` = `{slot, steamid?, score, kills, assists, downs, revives, headshots, pos?, ang?, health?, alive?, weapon?, clip?, ammo?, down, round, age_ms}`, sampled at the player's last input change, never after. The host decides what it means (lib/referee.js drop hold). *(added 2026-09-24, reconnect_rules.hpp)* |
| `player_back` | `slot, lost_ms, reseated?` | a `player_lost` slot's input moves again. A network blip unless `reseated:true` (the scoreboard counters went DOWN: the engine re-seated the slot for a new connection, i.e. the player relaunched). *(added 2026-09-24)* |
| `player_ready` | `slot` | a new connection's input is flowing: the player is past the loading screen. The host counts a returning player down only after this. *(added 2026-09-24)* |
| `restored` | `slot, ok, score?, kills?, downs?, not_restored?, error?` | a queued `restore` was applied (or could not be, or expired: `error`). `not_restored` lists what this DLL cannot put back yet (`weapons`, `perks`, `position`). *(added 2026-09-24)* |
| `down` / `revive` / `bleedout` | `slot` (+`by`) | `revive.by` is the **reviver** (whose `revives` counter moved); `revive.slot` is the one who got up, sent only when exactly one connected player was down *(DLL emits `revive` since 2026-09-23, referee.md §16)* |
...
| `stats` | `slot, score, kills, headshots, downs, revives, assists` | **the game's own Tab-scoreboard counters, absolute**, sent at connect and whenever kills/headshots/downs/revives/assists moves (after that frame's `down`/`revive` edge). The only source of kills and headshots from a real game: there is no kill event that names a player and `points` carries no `why`. Hosts fold each counter as a high-water mark. *(added 2026-09-23, referee.md §16)* |
...
| `dvar` | `name, value` | a watched dvar changed. Feeds manifest `{"dvar":…}` conditions and the replay's dvar log (vault 10 §4b). *(added 2026-09-20; **first actually sent 2026-09-23**: the 24 server dvars in `verified_env.hpp` `kServerWatch`, once at load and on every change, values as text in the engine's own format. `game_over` also carries the last values as `dvars:{…}`, and each player row `com_maxfps, com_maxfps_first, com_maxfps_changes` when reported. The host judges them: `lib/verified.js`, `docs/kickstart/verified-rules.md`)* |
| `client_dvar` | `slot, name, value, effective_fps` | a client's reported setting changed. Today only `name:"com_maxfps"`, read from the client's userinfo `enw_fps` (written by `client-dll/components/fps_guard.cpp`); `value` is the dvar (0 = uncapped), `effective_fps` what the engine runs at (`1000 / (1000 / value)`). Sent on first report and on change. *(added 2026-09-23)* |
...
| `snap` | `players:[{slot,pos:[x,y,z],ang:[pitch,yaw],health,score,weapon,stance,alive,clip?,ammo?}]`, `zombies:[{id,pos,yaw,health}]` | players 20 Hz; zombies 10 Hz, **20 Hz from replay-events-v1** (`map_loaded.replay_events >= 1`; either list may be omitted in a given snap). The gameplay events `weapon`, `fire`, `hit`, `damage`, `pap`, `powerup` and the `clip`/`ammo` fields: [`replay-events-v1.md`](replay-events-v1.md) *(added 2026-09-23, lane R1)* |
...
| `restart_request` | `slot, name, req, players, level_time` | a player pressed **Restart game** in the ENW Esc menu (the client sets userinfo `enw_req restart.<n>`; the dedi acts on a change only). The DLL does nothing else while a link is up: the host decides — the slot's identity is `verified`, or they are alone — and answers with `end {reason:"player_restart", match:<lease>}`, which ends the run as abandoned (`game_over` reason `player_restart`, then `match_end`) and restarts the map; the host moves the link to a new run `<lease>.r<n>` at `match_end`. Unknown to an older host, which ignores it (nothing restarts). *(added 2026-09-23, esc-menu.md §3)* |
...
| `expected_players` | `n` | *(2026-09-24, cloud-brief-parties.md task 1; dedi.md §30)* how many players the lease names. Sent when the link comes up, on every `map_loaded`, and when players are added to a running lease. The dedicated DLL (`expected_players.cpp`) answers `getnumexpectedplayers` with `max(1, n, clients connecting)` for 90 s after the map loads or a lease arrives, so round 1 waits for the whole party; then `max(1, connecting)`. Unknown to older DLLs, which ignore it. |
...
| `snapshot_state` | `id` | reply `value` = full restorable state (points, weapons, perks, position per player; round). The host asks for this the moment a player DROPS, while the level still has it. *(2026-09-24: a DLL with `map_loaded.reconnect` also lists players who already left, `connected:false`, with their state as of their last input -- the live read had nothing for them -- and marks a lost slot `lost:true`. Such a DLL sends the state on `player_lost` / `player_disconnect` itself, so a host need not ask.)* |
| `restore` | `id, slot, state` | put a returning player back as they were: `state` is that player's slice of an earlier `snapshot_state` (`score`, `weapon` incl. `_upgraded`, `perks`, `pos`, `ang`). Reply ok/error. *(2026-09-24, reconnect_rules.hpp: the DLL QUEUES it and replies at once with `value:{queued, will_restore:[score,kills,assists,downs,revives,headshots], not_restored:[weapons,perks,position]}`; it applies it once the slot's body has spawned and the world has run 1.5 s -- the level's own spawn script sets the rejoin score first -- by writing gclient +0x20BC..+0x20D0 (referee.md §16), and then sends `restored`. Score is put back exactly; the counters never go down. Refused with `reconnect off (ENW_NO_RECONNECT)`, `slot not connected`, `native client fields not verified`, `implausible counter in state`.)* **Only ever sent for casual/badge games** — a record-profile game gets the pause and a vanilla rejoin, because restoring by hand is not vanilla and would disqualify the run (vault 10 §5). *(added 2026-09-20)* |
