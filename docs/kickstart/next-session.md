...
| Site | live on main **`36193c4`** (restarted 22:53): 85 maps playable on our servers (was 64); GEO merged |
...
| Still running at handoff | **MAPS** (`aee8a57d93e4557fb`, 104 maps queued: 70 new + 34 re-proofs; unhides passes and appends boxProven.json — merge its branch + restart the site to make them "playable"), **S2** (`ab43c68ad02590cd9`, bot soaks alternating with MAPS; `snapacknowledged` fix for custom-map bot spawns unproven) |
...
| **Maps** | 78 listed; 5 five-gate proven with a real client (stock four + fear_mc_2); **59 more "New"** (boot on the box, no client has joined them); 5 of the popular 64 broken and hidden. *(Updated 2026-09-23 13:37 UK, lane A1 asset audit: **5 more hidden** in the live DB — dome_snow, snowglobe, nazi_zombie_test, nazi_zombie_test1, sanatorium — the release lacks a box weapon; fear_mc_2 has verdict `hide` but stays up for Z1/B; 50 live maps have no console log at all → `ZombiesDev\archive\reports\reproof-queue.txt`)* *(Updated 2026-09-23 23:55 UK, lane MAPS: Leviathan's `napalmblob` = its own precache of a weapon no zone defines, non-fatal since script_error_retail. **42 box passes** (185 s, watchdog clean): **17 new maps** incl. **Cheese Cube + Cheese Cube Unlimited** (B played Cheese Cube, round 5, clean), 5 hidden maps unhidden, Leviathan/MW2 Rust/Der Berg/DT2/Hijacked now box-proven; 15 box-pass but gate-hidden (§14.6). `boxProven.json` goes live with the site restart. **Resume:** `archive/resume-new-maps.txt` (62 new maps, all in the bucket + DB hidden) then `resume-reproof-t2.txt` (34, hold 125), §14.8)* | `dedi.md` §20, `archive.md` §10, §13, **§14** |
...
3. **zm_nuked hang (0.2.18)**: not reproduced on the box; the 0.2.19+ hang watchdog writes `hang-<pid>-<time>.dmp` + stack on the next one. `chat-overlay.md` §12.4. **2026-09-23 lane CL: very likely the GPU query wait of item 25 (zm_nuked has a sun flare); covered by `gpu_query_guard.cpp`.**
...
6. **Replay geometry**: Nacht's window walls from the Husky shell are missing (shots pass through); Der Riese has props only, the shell needs a Husky export under `game.lock`. `replay.md` §8.8, §8.11. **→ Superseded by item 33 (lane GEO): every map's shell now comes from the fastfile offline.**
...
26. **Lane CL (2026-09-23 ~16:45–18:15 UK, branch `worktree-agent-a022ed210c6d334f4`, main merged, not published): B's "crash launching Town of the Dead" was a HANG, found and fixed.** The render thread spins forever in the engine's GPU occlusion-query wait (`0x7255D0`: `while (GetData(FLUSH) == S_FALSE) Sleep(0)`, sun visibility `0x72C670`, only on maps whose world has a sun flare) because the query never answers on B's AMD driver (RX 9070 XT, `amdxn32.dll`); the main thread then waits on the render lock (`0x70E340` / CS `0x2298EA0`). Reproduced locally 4 of 5 guard-off runs on `zombie_town` (also with the chat overlay off), fixed by `gpu_query_guard.cpp` (the 4 call sites wait at most 50 ms, -1 = the engine's own no-answer, no waiting after 3 timeouts; `ENW_GPU_QUERY_GUARD=0` off): with the fix the guard tripped 0.4–0.6 s after the first frame and the game played on (90 s, twice, the second on the final DLL). Same class, covered without a list: `zm_nuked` (very likely item 3's 0.2.18 hang), `sanatorium`, `nazi_zombie_temple`, `nazi_zombie_puns`, `nazi_zombie_snowglobe`, `nazi_zombie_pogreb`, `nazi_zombie_rc`, stock Verrückt. Also: the hang watchdog names the render-lock holder and writes `hang_where` into `session-<pid>.json`; the launcher toasts "World at War froze on <map>. We have the logs." (it said nothing); `classifyGame` reads `session.exit`. **Client DLL `cc859f8a…` (sha256 `cc859f8ac8d0b2e0ec84f35d4c97a0d19982f9e29467bc1f869cdc00fd87fe3d`) for the next launcher publish** (`build/clfinal` in the worktree; no box deploy needed: client-only). Unproven: B's own Play with the fixed DLL; one post-fix run died in the map load from address-space exhaustion (the separate `client_memory_risk` class, `client.md` §13.7); whether the sun flare looks right after a trip; the hang dump itself still fails inside a real hang (`0x80070008`). `client.md` §13.

27. **Lane UGX (2026-09-23 ~17:00–18:40 UK, branch `worktree-agent-a6d057075d4bbb1bf`, main merged, NOT deployed): a map's own game mode is picked on the site and the in-game vote never shows.** Battlestar Galactica (B's `m_da684190`) is UGX Mod 1.0: `players[0] waittill("voting_complete")` on a `menuresponse` from menu `ugxm_vote_host` (`cl`/`gg`/`ar`/`ss`/`bh`, then `start`); no dvar exists. New server component `game_mode` hides the menu (detour on `PlayerCmd_OpenMenu` 0x4EF840) and sends the engine's own `menuresponse` notify as that player, from ONE host-owned dvar `enw_game_mode id:hide:menu:responses:done` (one, because the engine drops `+` commands past 31, `+map` included — measured, `Com_ParseCommandLine` 0x59AFC1). Scan of all 152 archive maps: **25 have a UGX vote** (16 UGX 1.0, 9 UGX 1.1), catalogue `web/server/data/map-modes.json`. Site: rail picker, `parties/assignments/games.game_mode`, boards per mode (`round@gungame`), mode tabs on the map page records tab, best round counts the default mode only, a mode the server did not prove is never a record. Proven locally in 3 modes + a control (no menu, `keyCatchers 0x0`, Gun Game / Sharpshooter HUDs in the captures, `ugxm_voting_complete` on the link) and the real events through the host Referee + site ingest. **Deploy: box DLL (rule 17) → host agent → site (additive migration runs itself) — order and box proof in `game-modes.md` §5.** Host lane follow-up: cap the server argv at 31 `+` commands (lease dvars first, never `+map`).

28. **Friends across ENW + live online list + launcher flash/chime (lane SOC, 2026-09-23 ~19:30 UK, branch `soc-friends`, main merged, not deployed)**: Movement's `friendships` imported read-only (`ssh webbox` + `sqlite3 -readonly`, both-ends-Zombies accepted pairs: 10 pairs, 7 of 9 accounts; drops.ws has no friend table; "ENW main server" unidentified, `questions.md` Q-soc-1). Rail: friends first then everyone, `Online / In launcher / In game on <map>, round N / In party`, pushed (`online_changed`, ~150 ms measured), friend-request block, party/DM in the chat dock. Launcher: flash until focus, one chime per burst (Notification sound, default on), toast with Accept (`enw-zombies://invite/<id>`), tray + taskbar dot, silent while a game runs. Needs: site.env `ZM_FRIENDS_MOVEMENT_SSH=webbox` + keepalive restart, client build + site restart on B's word, a launcher publish. Unproven: a real Electron window/toast, the rail in a browser unless `web.md` addendum says so. `web.md` + `launcher.md` "2026-09-23 ~17:40".

29. **Lane RV (2026-09-23 ~17:45–19:30 UK, branch `worktree-agent-af5a1466289b6332f`, main merged at `07d924a`, NOT deployed): replay sound for every replay, first-person arms + ADS, the fake gun, every stock gun in the pack.** B's "no sound" was the replays, not the audio code: measured with an AnalyserNode at the destination in headless Edge on main's build, `m_0c608cd9` (pre-v1 fear_mc_2, the one B opened at 18:07) had no AudioContext at all and `m_da684190`'s custom `m9` fired silent; the v1 Colt game played fine. Now old files sound from their attack presses/health drops, unknown guns use a same-class stock sound, a rebuilt sound engine stays enabled after the gesture. First person: the game's own `viewmodel_usa_marine_arms` holding each gun's viewmodel, posed by its idle anim and its ADS anim scrubbed by the aim fraction (the engine's rule), FOV to the gun's `adsZoomFov`; aim from the ADS button today, from the DLL's new `ads` field (`ps.fWeaponPosFrac`, `replay_events` 2, commit `5af3c78`, **needs a box DLL build**, `replay-events-v1.md` §8) once deployed. Any gun not in the pack is the procedural rifle with its real name on the tag, logged once. Pack +17 guns and their PaP, 17.4 MB: **already live** in `ZombiesDev\maps` (new files added, `_assets.json` backed up then replaced; main's viewer gets the new guns now). Proven: `replay-fp` 13/13 in `npm test`, `rv-render-check` 20/20, R3 27/27, R4 31/31. Unproven: heard by a person, the Electron window, the DLL field in a game. `replay.md` §14.

30. **Lane G2 (2026-09-23 ~17:00–19:30 UK, branch `worktree-agent-aac675948eb19e877`, main merged, NOT deployed): B's "one-hit downs" are a phantom water surface at z=0 on the dedicated server.** The server's water height comes from the renderer's water-sim window, which a dedi allocates (`watersim_pool.cpp`) but never fills, so every map whose floor is below 0 is under water on the server: zm_nuked (floor ~-390) drowned B at spawn (100→75→39→down in 150 ms, attacker none), nacht_reimagined (-87.6) had him swimming 35 units above the floor ("floating, missing inputs"). Fix `water_sim_off.cpp`: `r_gfxopt_water_simulation 0` on every dedi (the engine's static-grid path). Proven locally: nacht_reimagined spawns 100/100 and stands on -87.6 like a solo listen game. bridge_zombie/battlestar downs were stock (two 60-hits inside the 2.4 s regen). New every-map self-check `solo_parity.cpp` → `solo_parity: MISMATCH` → telemetry flag `solo_parity` (P2). **Follow-up 19:30:** zm_nuked's "stuck + death at +45 s" was the test client hanging (lane CL's GPU-query freeze, fixed by `gpu_query_guard` in launcher 0.2.29): with it Nuketown plays (stands on -398, two hits = down). ILS stands on its floor at -4.9 (not water), 58 Hz, no mismatch; its box lag is S2's. `dedi.md` §28, §28.8.

31. **Lane S2 (2026-09-23 ~17:00–19:00 UK, branch `worktree-agent-ab43c68ad02590cd9`, main merged): soak bots with no client anywhere, and the ILS lag is mostly CPU burnt by our own guarded reads under Wine.** `server/components/dedicated/bots.cpp`: SV_AddTestClient rebuilt from the engine's own functions (it is compiled out of T4 SP), bots stand, aim and fire, zombies killed through G_Damage from a bot so rounds advance (Nacht rounds 1→13 in 12 min, 0 escapes); `ENW_DEV_KNOBS=1` only (`ENW_DEV_BOTS=N`, `enw_dev_bots.txt`, host `settings.dev.bots`, `lease-cli --dev-bots N`); the referee ignores test clients. **Deploy-worthy on its own:** `shared/core/memory.cpp`'s image fast path — `perf` on an idle production ILS server: 0.82 core, 94% in Wine's VirtualQuery page-table scan (every guarded dword read), 1.2% in the game; with the fast path ILS runs 0.22 core at 61 Hz (B's ILS game ran its frame at ~24 Hz). Soaks run in the box TEST copy `waw-tinst-01` (`tools/dev/botrun.sh`/`botqueue.sh`, guard kills ours on any lease); the queue was blocked from 16:36 UTC by B's idle `ready` zm_nuked lease and restarts by itself. `dedi.md` §27. **Update 23:05 UK (all deployed: box DLL `59577dbe`, host `9e9e86a`):** soaks now run as host dev leases (`tools/dev/leasesoak.ps1`, `s2plan.ps1` alternates with MAPS). Bots on custom maps first spawned nothing: stock `wait_network_frame()` waits for the engine's per-packet `snapacknowledged` notify (SV_PacketEvent 0x635760), and bots send no packets; `bots.cpp` now raises it and acks every snapshot (§27.6). Proof: ut_box_map 30 min, rounds 1→16, 441 kills, 0 escapes, 61 Hz, 335 MB flat. The 36-min ILS soak `m_2a9a49f4` stayed at round 1 because it ran on the old image (`70b28f5b`); it was stable (0.21 core, 0 escapes). **Still to run:** Nacht / Der Riese 2 h to round 30+, lorkeep 30 min, ILS with rounds, 4 bots, rounds 20+ capacity. Commands in `dedi.md` §27.8.

32. **Restart + idle auto-close (lane RS, 2026-09-23 ~16:40–19:40 UK, branch `worktree-agent-a2f1f1baabf136fc8`, main merged, not shipped)**: B's bridge_zombie restart (m_abe60828) failed four ways: the console's confirm (2.1 s); a run that ends on its own took its server with it 268 ms after game over, and in solo the end_game comes a second after the down; **the console's first `restart` is never sent** (`setu` creates `enw_req` before it adds the userinfo flag; the Esc menu only worked because it changes `enw_ui` in the same frame); and **every `map_restart` on bridge_zombie faulted at `0x5AA0BD`** (SV_SpawnServer reads the UI's NULL `ui_gametype` pointer once the map has created that dvar by name — the §25/§26 class; it also breaks warm reuse there). Fixed: `restart` at once from the console; `enw_req` created at the first frame; the box DLL fills the `ui_gametype` slot; the host holds a naturally ended run for `--restart-grace-ms` (10 s) and a restart inside it starts `<lease>.r<n>` on the same process (`lease_continues` to the site); DLL debounce 3 s. Measured locally (real host agent, invisible client): **keypress → controllable 2.6–3.1 s on Nacht, 3.5–3.7 s on bridge_zombie**, live, while downed, after the game ended, and with spam. The rail's × now **closes the server and keeps the party** (`POST /api/party/end`, host only); Leave moved to the party header. **Idle auto-close** (B 19:10, after m_5a28dcbe sat empty 95 min): nobody admitted 5 min after ready, or everybody gone 3 min → the lease ends with reason `no_players` (host `lib/idle.js` + the referee's empty/crash-grace close; site `no_players` ack, `hold_idle` while a member downloads or resumes, launcher line *Server closed: nobody joined.*; Resume window now 3 min). **Deploy order: site, then host agent, then box DLL (rule 17)**; the client half (console, `enw_req`, the waiting-launcher line) needs a launcher release. `esc-menu.md` §12–§13.

33. **Lane GEO (2026-09-23 ~17:40–23:00 UK, branch `worktree-agent-aceedc562af7e9564`, main merged; the geometry is LIVE, the code is not merged): every map's replay geometry now comes out of its fastfile with no game.** Our OAT v0.33.0 build with a T4 GfxWorld dumper (`tools/maps/oat-t4-world`, GPL-3.0; built to `ZombiesDev\tools\oat-geo`) writes the shell, its materials and every static-model placement in about 1 s per map. On Nacht it matches Husky exactly (91 002 vertices, 1 506 smodels, window goals 57.3/61.3 u). Brush models are placed except doors, debris and boards. **236 exported, 235 pass align (float + served) and render, 235 promoted into `ZombiesDev\maps` (live via /mapdata)**. That is 95 of the 96 unhidden DB maps; `nazi_zombie_ali` has no fastfile and `nazi_zombie_mc_maze`'s window goals fail. Served median 6.92 MB, max 21.18 MB. fear_mc_2, bridge_zombie, water and pd are fixed (pd's "node 200 490 u out" was the sky dome). `mapAlign` reads meshopt/int16, so the 6.9 MB meshopt Nacht is live and `map-align.js` is 14/14. **IP: 231 custom maps' geometry is now public on gate-exempt /mapdata** (accepted by the coordinator for now; backups `maps\_work\*.pre-geo-*`). Every exporter holds `locks\heavy.lock` (`tools/maps/heavylock.py`). `replay.md` §15.
