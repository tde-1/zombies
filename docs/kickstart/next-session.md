...
|---|---|---|
| **Launcher on the feed** | *(updated 2026-09-23 12:36 UK)* **0.2.23**, client DLL **`06a2e1bd…`** = the box's binary (main **`f8bc2d9`**: lane 1's Discord hook guard, lane 12's menu/console lockdown; built clean in `wt-coord2`). `latest.yml` in the bucket and on `zombies.enw.gg/updates` says 0.2.23 | `088a38a`; `launcher.md` release table |
| **Box `zombies-dev`** | *(updated 2026-09-23 12:36 UK)* Box DLL **`06a2e1bd…`** in all 9 `pfx/drive_c/zdev/waw-*/binkw32.dll` (clean worktree `wt-coord2` at main **`f8bc2d9`**, installed 12:35 UK, boot proven by agent lease `m_2c3cf1b8`). Rollback `/home/waw/binkw32.rollback-499b70c1.dll` (chain since 03:30: `6b1ccfc5` → `683a475c` → `134a9d0f` → `499b70c1` → `06a2e1bd`). Host agent has the pull-on-lease map cache (15 GB budget). Host agent (systemd `enw-host-agent`, active) = several leases: **3 slots, 1 reserved for agent leases**, v2 pull protocol, `ready` at `map_loaded`, slot-based game copies and lobby ports (3074+slot), `lib/restart.js` (Esc menu Restart). `run-host.sh` exports **`ENW_NO_PAUSE=1`** (pause off) and **`ENW_DEDI_WATCH_PROBE_SLOT=1`** (data breakpoint on 0x3BFD478) | `dedi.md` §19, §21.5, **§22.2**; `host.md` §13; checked over ssh at 03:27 UK |
