...
|---|---|---|
| **Launcher on the feed** | *(updated 2026-09-23 13:49 UK)* **0.2.25**, client DLL **`974c2e8d…`** = the box's binary (main **`5696406`**: lane C1 console + settings everywhere + no paused start, 17b, T1, G1, A1; built clean in `wt-coord2`). `latest.yml` in the bucket and on `zombies.enw.gg/updates` says 0.2.25 | `fb902e9`; `launcher.md` release table |
| **Box `zombies-dev`** | *(updated 2026-09-23 13:49 UK)* Box DLL **`974c2e8d…`** in all 9 `pfx/drive_c/zdev/waw-*/binkw32.dll` (clean worktree `wt-coord2` at main **`5696406`**, installed 13:48 UK while B played; **no agent-lease boot proof yet**). Rollback `/home/waw/binkw32.rollback-10ba8544.dll` (chain since 03:30: `6b1ccfc5` → `683a475c` → `134a9d0f` → `499b70c1` → `06a2e1bd` → `10ba8544` → `974c2e8d`). **On B's word:** site restart (telemetry, settingsRepairs, mapfiles fixes) and host-agent tar redeploy + restart (telemetry bundles), then `box_stage.py --map neon_fighter --add-missing`. Host agent has the pull-on-lease map cache (15 GB budget). Host agent (systemd `enw-host-agent`, active) = several leases: **3 slots, 1 reserved for agent leases**, v2 pull protocol, `ready` at `map_loaded`, slot-based game copies and lobby ports (3074+slot), `lib/restart.js` (Esc menu Restart). `run-host.sh` exports **`ENW_NO_PAUSE=1`** (pause off) and **`ENW_DEDI_WATCH_PROBE_SLOT=1`** (data breakpoint on 0x3BFD478) | `dedi.md` §19, §21.5, **§22.2**; `host.md` §13; checked over ssh at 03:27 UK |
...
21. **Replay FX (lane R3, 2026-09-23 ~15:30 UK, branch `worktree-agent-aef74e453358c99cc`, main merged, not deployed)**: the web replay now draws R2's weapons in the hands (gold PaP models), muzzle flashes, the game's hit marker and hurt vignette, the crosshair in third person too, power-up models with timed chips in tenths, and plays R2's sounds (off until Play; M mutes). It is built on R1's v1 events and asks for the track at 20 Hz. Proven on a signed fixture plus R2's real pack on a scratch site (render check 27/27, `replay-fx` 16/16). Old replays play silent with nothing new. **Unproven until a real v1 game exists** (R1's DLL is not deployed) and until someone listens on real speakers. Live on merge plus a site restart on B's word. `replay.md` §12.
...
1. Update to **0.2.25** (the chip top right, or restart the launcher). *(2026-09-23 13:49 UK)* On its first launch it repairs the Discord overlay and dual-video-card (`r_multiGpu`) settings by itself. In the game's console, `/quit` and `fov 90` now work (Tab completes), and a map no longer starts paused under its start menu.
