...
28. **Restart (lane RS, 2026-09-23 ~18:30 UK, branch `worktree-agent-a2f1f1baabf136fc8`, main merged, not shipped)**: B's bridge_zombie restart (m_abe60828) failed three ways: the console's confirm (2.1 s); a run that ends on its own took its server with it 268 ms after game over (`--after-game terminate`), and in solo the end_game comes a second after the down; and **the console's first `restart` is never sent** (`setu` creates `enw_req` before it adds the userinfo flag; the Esc menu only worked because it changes `enw_ui` in the same frame). Fixed: `restart` at once from the console; `enw_req` created at the first frame; the host holds a naturally ended run for `--restart-grace-ms` (10 s) and a restart inside it starts `<lease>.r<n>` on the same process (`lease_continues` to the site); DLL debounce 3 s. Measured locally (real host agent, invisible client): **keypress → controllable ≈ 2.9 s** live, while downed and after the game ended. The rail's × now **closes the server and keeps the party** (`POST /api/party/end`, host only); Leave moved to the party header. **Deploy order: site, then host agent** (a host posting `lease_continues` to an old site loses the lease); the client half needs a launcher release; box DLL optional (debounce). `esc-menu.md` §12.
