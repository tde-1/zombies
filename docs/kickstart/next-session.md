...
6. **Replay geometry**: Nacht's window walls from the Husky shell are missing (shots pass through); Der Riese has props only, the shell needs a Husky export under `game.lock`. `replay.md` §8.8, §8.11. **→ Superseded by item 28 (lane GEO): every map's shell now comes from the fastfile offline.**
