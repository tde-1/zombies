...
2. ~~**B's client writes no `console.log` since 2026-09-22 18:07**~~ *(2026-09-23 ~04:00: wrong premise - it is written per map and truncated per launch; the DLL now keeps `logsnsole-<pid>.log`, `client.md` "2026-09-23 ~04:00". Chat-Enter crash = Discord hook, `chat-overlay.md` §13.)* despite `+set logfile 2`, so B's reports have no engine evidence. `mod-compat.md` §1, `client.md` "2026-09-23 01:45".
