...
## 16. 2026-09-26 — the cloud run: what ran, what is waiting, and how B's PC publishes it

The cloud session's network was opened by B, so §15.4's FETCH lane moved there. Work dir
`/home/user/zwork` (catalogue, cache, originals; lost when the container is reclaimed; the
state that matters goes to the bucket under `archive/state/`).

**Proven in the cloud:** zwr.gg, callofdutyrepo, UGX-Mods and archive.org crawl from it
(ModDB and MediaFire's home page answer a Cloudflare challenge; MediaFire *file* pages work).
MEGA for real: `7734.exe` 180,836,505 B in 10 s, meta-MAC ok. The pipeline end to end
(`cloud_pipeline.py`) on `1`, `angst`, `beartown`: originals + `mods/<bsp>/` in the bucket, a
public GET of `mods/nazi_zombie_bear_town/nazi_zombie_feto.ff` matches extract.json's sha256.
Static checks per map on Linux (`cloud_static.py`: scan, precheck, modes; asset audit recorded
`unproven`; manifests start `site_hidden`).

**Numbers (05:30 UTC):** catalogue 2,024 distinct maps; 263 already held by B (matched on the
committed manifests); 70 fetched in the cloud (21 GB; 3 uploaded); ~820 still to fetch with a
robot link; 19 MEGA takedowns seen at download time (now recorded as dead links); callofdutyrepo
pass C resumed after a 403 at 03:21 (the crawler now skips one broken post instead of stopping).
Browser checklist: `archive/browser-queue-2026-09-26.md` (22 links, 400 most-wanted titles).

**Blocked:** the unattended upload loop (`archive/cloud_loop.sh`) was refused by the session's
auto-mode classifier as a shared-resource write; it needs B's permission rule
`Bash(/home/user/zombies/archive/cloud_loop.sh)`. Until then the cloud can only fetch to its own
~30 GB disk.

**Found on the way (fixed):** `lib/net.py`'s pid check was Windows-only (a stale host lock
blocked a host for 30 min on Linux); `avscan.py` would have thrown on Linux after a successful
download; `extract.py` hardcoded 7-Zip's Windows path; `scan_maps`/`precheck` globbed `.ff`
case-sensitively off Windows.

**Needs B's PC once:** `python archive/scan_maps.py --keep-existing` writes
`archive/corpus-ignore.json` (the shared-name list the full scan subtracts); commit it, or every
cloud verdict other than `round` stays provisional.

### 16.1 Publishing cloud maps from B's PC

The site serves only files it holds locally (`web/server/lib/mapfiles.js:55-68`) and the box's
map cache takes its file list from the site (`infra/host-agent/host.js:978`), so a cloud map is
pulled to the PC first. `import-archive.js` reads manifests only (`:131-134`).

```
git pull
python archive\merge_cloud.py --dry --pull-mods      & rem counts, conflicts
python archive\merge_cloud.py --pull-mods            & rem extract.json, cloud-maps.txt, sidecars, mods\<bsp> (sha-checked)
rem read reports\cloud-merge-conflicts.json first: each is a bsp needing a human
python archive\scan_maps.py --keep-existing          & rem manifests missing from git; also corpus-ignore.json
python archive\tranche.py manifests --list archive\cloud-maps.txt --tranche cloud1
rem back up web\data, then
set ZM_ARCHIVE_MANIFESTS=C:\Users\b\Desktop\Zombies\archive\manifests
node web\server\db\import-archive.js --maps-list archive\cloud-maps.txt
python archive\box_proof.py --map-list archive\cloud-maps.txt --player 76561198000000005 --lease-repo C:\Users\b\Desktop\Zombies --hold 185 --load-wait 300 --wait-busy 14400 --skip-done --save-console C:\Users\b\ZombiesDev\archive\logs\box-console\proof-cloud-<date>
rem append proven keys to archive\applied-maps-lane.txt, then
python archive\popular.py --apply --only archive\applied-maps-lane.txt
node web\server\db\import-archive.js --maps-list archive\applied-maps-lane.txt
```
MEGA-sourced maps: `box_stage.py`'s default path re-downloads from MediaFire only; the map cache
(or `--stage-from-bucket`) is the way, same bytes. Then geometry (`replay.md` §15) for each new bsp.
