# STATUS history (moved out of STATUS.md on 2026-09-24; kept as written)
...
| | State | Pointer |
|---|---|---|
| **Launcher on the feed** | **0.2.35** (2026-09-23 20:10 UK, lane REL), client DLL **`1b482aa2`** (clean `ZombiesDev\wt-rel8` at main `c99b346`; same binary as the 7 box copies): + lane RS (instant console restart, first-restart setup fix, dedi `ui_gametype` fix, party close/leave, idle auto-close) + G2 water_sim_off race fix. Before it, 0.2.34 `3557aaa3`: CL `gpu_query_guard` (the zombie_town / Town of the Dead hang on AMD) + "World at War froze/crashed on <map>" notice + `hang_where`; UGX `game_mode`; SOC flash/chime/toast + `notifySound`; RV `replay_events` 2 (`ads`); G2 `water_sim_off` + `solo_parity` (dedi-only); S2 image fast path + dev bots. Same evening: 0.2.29 `0d9125d6`, 0.2.30 `3970122b`, 0.2.31 `6cbf8566`, 0.2.32 `2fda99fe`, 0.2.33 `736236c8`. Before: 0.2.28 `fd3039d2` | `launcher.md` release table |
| **Box DLL** | **`59577dbe`** in the 7 production copies since 20:27 UTC (lane REL; clean `ZombiesDev\wt-rel10` at `4ead149`, built `/m:2`: + S2 `52b2169` level notify `snapacknowledged` once per server frame while bots exist, `ENW_DEV_KNOBS=1` only; rollback `binkw32.rollback-70b28f5b.dll`; not yet run by a new game). Before it: **`70b28f5b`** in the 7 production copies since 19:34 UTC (lane REL; clean `ZombiesDev\wt-rel9` at local main `d8b580c`: + S2 `533cdae` dev-bot fixes, all behind `ENW_DEV_KNOBS=1`, so launcher 0.2.35 `1b482aa2` stays the client; rollback `binkw32.rollback-1b482aa2.dll`; first new game on it pending at 19:34 UTC (the next lease, MAPS zombie_maze, was waiting on the RAM guard)). Before it: **`1b482aa2`** in the 7 production copies (`waw-inst-01..04`, probe, stock, vps1; 2026-09-23 18:58 UTC, lane REL, in a gap with no game; clean `ZombiesDev\wt-rel8` at main `c99b346`) = launcher 0.2.35. `waw-tinst-*` untouched. Proven: nacht_reimagined `m_35b82cb2` + nazi_zombie_ils `m_4bed9049`: `dedi_water_sim_off: post_init: r_gfxopt_water_simulation is already 0 … held at 0` (G2 fix; never `NOT applied`), 0 real `solo_parity: MISMATCH`, `restart_request: armed` + `ui_gametype` slot filled; ILS **17.9 % of one core over 60 s, 60.9 Hz** (nacht_reimagined 17.6 %, 60.9 Hz). Rollbacks in `/home/waw/`: `binkw32.rollback-3557aaa3.dll` (0.2.34, water fix raced), `-736236c8`, `-884dde5f`, `-2fda99fe`, `-fd3039d2` | `dedi.md` §26.6–26.7 |
| ~~Box DLL (INT)~~ | `fd3039d2` in all 9 copies 16:00–18:17 UTC (lane INT, clean `wt-int` at `fa1784f`): S1 + INT NULL-dvar registrations, F1 rate x4. Still in `waw-tinst-02` | `dedi.md` §26 |
| ~~Box DLL (before INT)~~ | **`04a3ad6d`** in all 9 copies (2026-09-23 14:13 UK; clean worktree `wt-coord2` at main `bde7e19`: D1 freeze watchdog + dedi `_load` zone, R1 replay events at 20 Hz). Rollbacks `binkw32.rollback-1fda51c5.dll` (D1-only, 14:10–14:13) and `binkw32.rollback-974c2e8d.dll`. Proven: Nacht `m_7ce70442` (replay header `replay_events:1`, `snap_hz:20`), ray_chirstmas_map `m_d7129f0e` on `1fda51c5` (`dedi_load_zone` armed, 9 missing xmodels, none a zombie part). Chain: … → `10ba8544` → `974c2e8d` → `1fda51c5` → `04a3ad6d` | `dedi.md` §22.6–22.9 |
| **Box host agent** | **Main `a2c330d` deployed 19:12 UTC** (lane REL; rollback `/home/waw/host-agent.rollback-20260923T1912Z.tgz`): S2 `3b5ffd8` (a bot soak lease is not closed as empty). Before it main `c99b346` 18:58 UTC (lane RS: restart path `lib/restart.js`, idle auto-close `lib/idle.js`; rollback `host-agent.rollback-20260923T1857Z.tgz`) and **main `9589c91` deployed 18:27 UTC** (lane REL, in a gap in lane MAPS's queue; rollback `/home/waw/host-agent.rollback-20260923T1826Z.tgz`): UGX `gamemode.js` (+ referee `game_mode` result), the 31 `+`-command guard (a line that would lose `+map` is refused with the reason), S2 `settings.dev.bots`. Before it: **main `bde7e19` deployed 14:14 UK** (whole `infra/host-agent` tree minus test/, + `referee/manifests`, chown waw; rollback `/home/waw/host-agent.rollback-20260923T1314Z.tgz`; before it `dcf0ee7` 14:06): D1 game flags on the record, R1 `replay_events`/`snap_hz` in the replay header,: H1 boot queue (one boot at a time: `queued to boot, 1 ahead` seen), RAM guard (floor 700 MB), telemetry ON (host `instance_end` + daily journal bundles reach the site and `logs/` in the bucket), pull-on-lease map cache (59 dirs, 14.8/15 GB). 3 slots, 1 reserved for agents; v2 pull; `ready` at `map_loaded`. `run-host.sh`: `--after-game terminate`, `ENW_NO_PAUSE=1`, `ENW_DEDI_WATCH_PROBE_SLOT=1` | `host.md` §13, `dedi.md` §18.6, §19, §21.3 |
| **Site** | Live on B's PC; keepalive loop detached + Startup shortcut. **Main `c99b346` live 19:52 UK** (lane REL: main checkout fast-forwarded, `npm run build`, node on 3200 killed, keepalive restarted it; after the coordinator's own restart on `b622811` with the friends env + DB backup): all the evening merges incl. lane RS. **Coordinator 20:0x: the live bundle hangs (React/socket update loop); lane SOC is fixing and restarting it.** **Main (lane INT, 2026-09-23 ~17:10 UK) live**: R4 replay spectator switching (client rebuilt), L1 telemetry rules (`server_freeze` P1, known fault names), `infra/discord.env` read by node at start (B's Discord app id reaches the launcher hello; restarted with nobody live). Before it: **main `4b68d65` live since 14:29 UK** (R3 replay viewer: weapons, muzzle flash, hit markers, vignette, PaP, power-ups, sounds, 20 Hz; `/mapdata/_assets.json` 200); since 13:54: telemetry (`incidents` table, `/api/gs/telemetry`, admin Issues), settingsRepairs (`repair: r_multiGpu 1 -> 0 (old default) on 0 accounts`), mapfiles serve fixes, H1 queued phase + admin Boxes RAM column. First real bundles 14:06 (incidents 1–3, all `uploaded`) | `infra\keepalive.log`, `web.md` |
