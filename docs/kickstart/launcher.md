...
| **0.2.36** | `380cda5` 2026-09-24 01:24 (main `3d5ce16` DLL source; DLL clean in `ZombiesDev\wt-rel11`) | **`a02958f8`** | Lane SS: ENW's own F12 screenshot (`screenshot.cpp`: WaW's screenshotJPEG redirected, JPEG q95 4:4:4 / PNG to `Pictures\ENW Zombies`, "Screenshot saved"), launcher Settings > Screenshots (Open image / Show in folder / Open screenshots folder) + post-game toast; includes CE's `screenshot_guard` + Documents redirect. run-all 193/0, waw-settings 20/0, C++ 27/69/200 all 0 failed; feed `latest.yml` 0.2.36 (bucket + site, `text/yaml`) | `client.md` §15, this doc 2026-09-24 SS |
