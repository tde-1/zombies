...
| | State | Pointer |
|---|---|---|
| **Launcher on the feed** | **0.2.34** (2026-09-23 19:31 UK, lane REL), client DLL **`3557aaa3`** (clean `ZombiesDev\wt-rel7` at main `0a03304`; same binary as 7 box copies): CL `gpu_query_guard` (the zombie_town / Town of the Dead hang on AMD) + "World at War froze/crashed on <map>" notice + `hang_where`; UGX `game_mode`; SOC flash/chime/toast + `notifySound`; RV `replay_events` 2 (`ads`); G2 `water_sim_off` + `solo_parity` (dedi-only); S2 image fast path + dev bots. Same evening: 0.2.29 `0d9125d6`, 0.2.30 `3970122b`, 0.2.31 `6cbf8566`, 0.2.32 `2fda99fe`, 0.2.33 `736236c8`. Before: 0.2.28 `fd3039d2` | `launcher.md` release table |
| **Box DLL** | **`3557aaa3`** in the 7 production copies (`waw-inst-01..04`, probe, stock, vps1; 2026-09-23 18:29–18:35 UTC, lane REL; clean `ZombiesDev\wt-rel7` at main `0a03304`) = launcher 0.2.34. `waw-tinst-*` untouched (S2/other lanes). **REGRESSION: G2 `dedi_water_sim_off` is NOT applied on it** (`[0x042B721C]=00000000 but Dvar_FindVar(r_gfxopt_water_simulation)=021BAC04` on nacht_reimagined and derberg; no line at all on ILS). It WAS applied on `884dde5f` (G2 without S2: nacht_reimagined `r_gfxopt_water_simulation 1 -> 0`), and fails on every S2 build (`736236c8`, `3557aaa3`) -- see `dedi.md` §26.6. S2 proven: nazi_zombie_ils agent lease 16.4 % of one core over 60 s, `dedi_rate_probe` 61 Hz. Rollbacks in `/home/waw/`: `binkw32.rollback-884dde5f.dll` (water fix works, no S2), `-2fda99fe`, `-736236c8`, `-fd3039d2` (INT) | `dedi.md` §26.6 |
