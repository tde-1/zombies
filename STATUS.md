...
| **Site** | Live on B's PC; keepalive loop detached + Startup shortcut. **Main `dcf0ee7` live since 13:54 UK** (bundle built 13:54): telemetry (`incidents` table, `/api/gs/telemetry`, admin Issues), settingsRepairs (`repair: r_multiGpu 1 -> 0 (old default) on 0 accounts`), mapfiles serve fixes, H1 queued phase + admin Boxes RAM column. First real bundles 14:06 (incidents 1–3, all `uploaded`) | `infra\keepalive.log`, `web.md` |
