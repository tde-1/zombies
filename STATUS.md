...
ame x`), a second client's scoreboard, and the picker has no UI yet (API + `needs_name` live). `SV_ExecuteClientCommand` corrected to 0x6308F0; no `CS_PLAYERS` configstring in this exe. **0.2.6 not yet published** (shell + name lock + `+name` are committed, unversioned). Vault
