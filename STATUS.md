...
| | State | Pointer |
|---|---|---|
| **Launcher on the feed** | **0.2.25** (2026-09-23 13:49 UK), client DLL **`974c2e8d`** (main `5696406`, same binary as the box). Tonight: 0.2.7 → 0.2.25 | `launcher.md` release table |
| **Box DLL** | **`974c2e8d`** in all 9 copies (2026-09-23 13:48 UK; clean worktree `wt-coord2` at main `5696406`: C1, 17b, T1, G1, A1). Rollback `binkw32.rollback-10ba8544.dll`. Boot proven 14:06 UK: agent leases `m_20f02b99` + `m_4d8ef762` (Nacht, `map_loaded`) and `m_af069460` (neon_fighter, `map_loaded`). Chain: `b36fe140` → `f8a835bb` → `86f12b12` → `6fccc0e0` → `79d4317d` → `c0986e5e` → `f920bb39` (unrecorded, reverted) → `03b04bc3` → `6b1ccfc5` → `683a475c` → `134a9d0f` → `499b70c1` → `06a2e1bd` → `10ba8544` → `974c2e8d` | `dedi.md` §22.6–22.9 |
| **Box host agent** | **Main `dcf0ee7` deployed 14:06 UK** (whole `infra/host-agent` tree minus test/, + `referee/manifests`, chown waw; rollback `/home/waw/host-agent.rollback-20260923T1305Z.tgz`): H1 boot queue (one boot at a time: `queued to boot, 1 ahead` seen), RAM guard (floor 700 MB), telemetry ON (host `instance_end` + daily journal bundles reach the site and `logs/` in the bucket), pull-on-lease map cache (59 dirs, 14.8/15 GB). 3 slots, 1 reserved for agents; v2 pull; `ready` at `map_loaded`. `run-host.sh`: `--after-game terminate`, `ENW_NO_PAUSE=1`, `ENW_DEDI_WATCH_PROBE_SLOT=1` | `host.md` §13, `dedi.md` §18.6, §19, §21.3 |
