...
| | State | Pointer |
|---|---|---|
| **Launcher on the feed** | **0.2.30** (2026-09-23 18:44 UK, lane REL), client DLL **`3970122b`** (clean `ZombiesDev\wt-rel2` at main `07d924a`): lane CL `gpu_query_guard` (the zombie_town / Town of the Dead hang on AMD), "World at War froze/crashed on <map>" notice, `hang_where`; lane UGX `game_mode` (server-side). 0.2.29 `0d9125d6` (CL only) was on the feed 10 min. Before: 0.2.28 `fd3039d2` | `launcher.md` release table |
| **Box DLL** | **`fd3039d2`** in all 9 copies (2026-09-23 16:00 UTC, lane INT; clean `ZombiesDev\wt-int` at `fa1784f` = main `645649c` DLL source): S1 `snd_errorOnMissing`/`snd_reportSndAliasErrors` + INT `r_watersim_debug`/`fx_enable` registered on the dedi (B's 5 `server_freeze` games), F1 rate scale x4, fault names. Proven: ut_box_map, nazi_zombie_ils ×2, nazi_zombie_lorkeep agent leases ≥3 min, 0 escape faults. Rollbacks `binkw32.rollback-cc052624.dll` (15:51–16:00), `binkw32.rollback-04a3ad6d.dll` (pre-INT) | `dedi.md` §26 |
